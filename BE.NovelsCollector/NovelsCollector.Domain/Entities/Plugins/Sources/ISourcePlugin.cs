...
namespace NovelsCollector.Domain.Entities.Plugins.Sources
{
    public interface ISourcePlugin : IPlugin
    {
        ...
        public Task<Tuple<INovel[]?, int>> CrawlSearch(string? query, int page = 1);
        public Task<Tuple<INovel[]?, int>> CrawlQuickSearch(string? query, int page = 1);

        // Get novel information
        public Task<Novel?> CrawlDetail(string novelSlug);

        // Get list of chapters
        public Task<Tuple<IChapter[]?, int>> CrawlListChapters(string novelSlug, int page = -1);

        // Get chapter content
        public Task<IChapter?> CrawlChapter(string novelSlug, string chapterSlug);
        public Task<IChapter?> GetChapterAddrByNumber(string novelSlug, int chapterNumber);

        // Get list of categories
        public Task<ICategory[]> CrawlCategories();

        // Get novels by category
        public Task<Tuple<INovel[], int>> CrawlByCategory(string categorySlug, int page = 1);
