...
{
    public class GetCategoryNovelsUC
    {
        private readonly IEnumerable<ISourcePlugin> _plugins;

        public GetCategoryNovelsUC(IEnumerable<ISourcePlugin> plugins) => _plugins = plugins;

        public async Task<Tuple<Novel[], int>> Execute(string source, string categorySlug, int page = 1)
        {
            Novel[]? novels = null;
            int totalPage = -1;
