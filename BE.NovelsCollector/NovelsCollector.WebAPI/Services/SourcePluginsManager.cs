...
using NovelsCollector.Aplication.Services;
...
{
    public class SourcePluginsManager : BasePluginsManager<SourcePlugin, ISourceFeature>, ISourcePluginManager
    {
        ...
        public SourcePluginsManager(IPluginRepository<SourcePlugin> myMongoRepository, IMemoryCache cacheServide, ILogger<SourcePluginsManager> logger)
            : base(myMongoRepository, pluginsFolderName)
        {
            _logger = logger;
        ...
        public async Task<Tuple<Novel[], int>> GetNovelsByCategory(string source, string categorySlug, int page = 1)
        {
            ...
            if (page == 1)
            {
                ...
                if (_cacheService.TryGetValue(cacheKey, out novels) && _cacheService.TryGetValue(cacheKeyTotalPage, out totalPage))
                {
                    ...
                    return new Tuple<Novel[], int>((Novel[])novels, totalPage);
                }
            }
